            ...
            <div class="container-fluid">
                ...
                <div class="card">
                    <div class="card-header">
                        <i class="fa fa-align-justify"></i> Ordenes de servicio por cobrar
                                ...
                                <tbody>
                                    <tr v-for="ordenservicio in arrayOrdenservicio" :key="ordenservicio.id">
                                        <td>
                                            ...
                                            <template v-if="ordenservicio.estadoSaldo==0.00">
                                                <button type="button" class="btn btn-info btn-sm" @click="desactivarOrden(ordenservicio.id)">
                                                    <i class="icon-doc">Váucher</i>
                                                </button>
                                            </template>
                                        ...
                                        <td>
                                        <div v-if="ordenservicio.estadoSaldo == '1.00'">
    ...
    export default {
        ...
        methods : {
            ...
            listarOrdenServicio (page,buscar,criterio){
                let me=this;
                var url= '/ordenservicio/obtenerOrdenesFinalizadas?page=' + page + '&buscar='+ buscar + '&criterio='+ criterio;
                axios.get(url).then(function (response) {
                    var respuesta= response.data;
                    me.arrayOrdenservicio = respuesta.ordenesFinalizadas.data;
            ...
            generarPDF(id){
                window.open('http://127.0.0.1:8000/ordenservicio/generarPDF?idOrden='+id, '_blank');
            },
            desactivarOrden(id){
               swal({
                title: '¿Desea pagar esta orden?',
                type: 'warning',
                ...
                if (result.value) {
                    let me = this;
                    axios.put('/ordenservicio/pagado',{
                        'id': id,
                    }).then(function (response) {
                        //me.listarOrdenServicio(1,'','noDocumento');
                        me.generarPDF(id);
                        swal(
                        '¡Pagado!',
                        'La orden de servicio ha sido cobrada exitosamente',
                        'success'
                        )
